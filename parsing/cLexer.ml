...
let to_coqloc loc =
  {Loc.fname = coq_file_of_ploc_file (Ploc.file_name loc);
   Loc.line_nb = Ploc.line_nb loc; Loc.bol_pos = Ploc.bol_pos loc;
   Loc.bp = Ploc.first_pos loc; Loc.ep = Ploc.last_pos loc;
...
module CharOrd =
  struct
    type t = char
    let compare : char -> char -> int = compare
  end
...
let ttree_remove ttree str =
  let rec remove tt i =
    if i == String.length str then {node = None; branch = tt.branch}
    else
      let c = str.[i] in
      let br =
        match try Some (CharMap.find c tt.branch) with Not_found -> None with
...
let set_loc_pos loc bp ep = Ploc.sub loc (bp - Ploc.first_pos loc) (ep - bp)
...
let error_utf8 loc cs =
  ...
  Stream.junk cs;
  (* consume the char to avoid read it and fail again *)
  let loc = set_loc_pos loc bp (bp + 1) in err loc Illegal_character

let utf8_char_size loc cs =
  function
    '\x00'..'\x7F' -> 1
  | '\xC0'..'\xDF' -> 2
  | '\xE0'..'\xEF' -> 3
...
let lookup_utf8_tail loc c cs =
  ...
  if Int.equal (c1 land 0x40) 0 || Int.equal (c1 land 0x38) 0x38 then
    error_utf8 loc cs
  else
    let (n, unicode) =
      if Int.equal (c1 land 0x20) 0 then
        ...
        match Stream.npeek 3 cs with
          [_; c2; c3] ->
            ...
            3,
            (c1 land 0x0F) lsl 12 + (Char.code c2 land 0x3F) lsl 6 +
            Char.code c3 land 0x3F
        | _ -> error_utf8 loc cs
      else
        match Stream.npeek 4 cs with
          [_; c2; c3; c4] ->
            check_utf8_trailing_byte loc cs c2;
            check_utf8_trailing_byte loc cs c3;
            check_utf8_trailing_byte loc cs c4;
            4,
            (c1 land 0x07) lsl 18 + (Char.code c2 land 0x3F) lsl 12 +
...
let check_keyword str =
  let rec loop_symb (strm__ : _ Stream.t) =
    match Stream.peek strm__ with
      Some (' ' | '\n' | '\r' | '\t' | '"') ->
        Stream.junk strm__; bad_token str
    | _ ->
        let s = strm__ in
...
let check_ident str =
  let rec loop_id intail (strm__ : _ Stream.t) =
    match Stream.peek strm__ with
      Some ('a'..'z' | 'A'..'Z' | '_') ->
        Stream.junk strm__; loop_id true strm__
    ...
    | _ ->
        ...
        match unlocated lookup_utf8 Ploc.dummy s with
          Utf8Token (st, n)
          when not intail && Unicode.is_valid_ident_initial st ->
            njunk n s; loop_id true s
        | Utf8Token (st, n)
          when intail && Unicode.is_valid_ident_trailing st ->
            njunk n s; loop_id true s
        | EmptyStream -> ()
...
let is_ident str = try let _ = check_ident str in true with Error.E _ -> false
...
let is_keyword s =
  try
    match (ttree_find !token_tree s).node with
      None -> false
    | Some _ -> true
...
let add_keyword str =
  if not (is_keyword str) then
    begin check_keyword str; token_tree := ttree_add !token_tree str end

let remove_keyword str = token_tree := ttree_remove !token_tree str
...
let store len x =
  let open Bytes in
  begin
    if len >= length !buff then buff := cat !buff (create (length !buff));
    set !buff len x;
    succ len
  end
...
let warn_unrecognized_unicode =
  CWarnings.create ~name:"unrecognized-unicode" ~category:"parsing"
    (fun (u, id) ->
       strbrk
         (Printf.sprintf
            "Not considering unicode character \"%s\" of unknown \
                                  lexical status as part of identifier \"%s\"."
            u id))

let rec ident_tail loc len (strm__ : _ Stream.t) =
  match Stream.peek strm__ with
    Some ('a'..'z' | 'A'..'Z' | '0'..'9' | '\'' | '_' as c) ->
      Stream.junk strm__; ident_tail loc (store len c) strm__
  | _ ->
      let s = strm__ in
      ...
      | Utf8Token (st, n) when Unicode.is_unknown st ->
          ...
          let u =
            String.concat "" (List.map (String.make 1) (Stream.npeek n s))
          in
...
let warn_comment_terminator_in_string =
  CWarnings.create ~name:"comment-terminator-in-string" ~category:"parsing"
    (fun () ->
       strbrk "Not interpreting \"*)\" as the end of current \
...
let rec string loc ~comm_level bp len (strm__ : _ Stream.t) =
  match Stream.peek strm__ with
    Some '"' ->
      Stream.junk strm__;
      let esc =
  ...
  | Some '(' ->
      ...
      (fun (strm__ : _ Stream.t) ->
         ...
         | _ -> string loc ~comm_level bp (store len '(') s)
        s
  | Some '*' ->
      Stream.junk strm__;
      let s = strm__ in
  ...
  | Some c ->
      Stream.junk strm__;
      let s = strm__ in string loc ~comm_level bp (store len c) s
  | _ ->
      let _ = Stream.empty strm__ in
      let ep = Stream.count strm__ in
      let loc = set_loc_pos loc bp ep in err loc Unterminated_string
...
type lexer_state =
  int option * string * bool * ((int * int) * string) list * Loc.source
...
let set_lexer_state (o, s, b, c, f) =
  ...
  current_file := f
let get_lexer_state () =
  !comment_begin, Buffer.contents current_comment, !between_commands,
  !comments, !current_file
let release_lexer_state = get_lexer_state
let drop_lexer_state () = set_lexer_state (init_lexer_state Loc.ToplevelInput)
...
let push_char c =
  if !between_commands || List.mem c ['\n'; '\r'] ||
     List.mem c [' '; '\t'] &&
     (Int.equal (Buffer.length current_comment) 0 ||
      not
        (let s = Buffer.contents current_comment in
...
let comment_stop ep =
  ...
  if !(Flags.beautify) && Buffer.length current_comment > 0 &&
     (!between_commands || not (null_comment current_s))
  then
    begin let bp =
      match !comment_begin with
        Some bp -> bp
...
let rec comment loc bp (strm__ : _ Stream.t) =
  let bp2 = Stream.count strm__ in
  match Stream.peek strm__ with
    Some '(' ->
      Stream.junk strm__;
      let loc =
        try
          match Stream.peek strm__ with
            Some '*' ->
              Stream.junk strm__;
              let s = strm__ in push_string "(*"; comment loc bp s
          | _ -> push_string "("; loc
        with Stream.Failure -> raise (Stream.Error "")
      in
      comment loc bp strm__
  | Some '*' ->
      Stream.junk strm__;
      begin try
        match Stream.peek strm__ with
          Some ')' -> Stream.junk strm__; push_string "*)"; loc
        | _ -> let s = strm__ in real_push_char '*'; comment loc bp s
      with Stream.Failure -> raise (Stream.Error "")
      end
  | Some '"' ->
      Stream.junk strm__;
  ...
  | _ ->
      match try Some (Stream.empty strm__) with Stream.Failure -> None with
        Some _ ->
          let ep = Stream.count strm__ in
          let loc = set_loc_pos loc bp ep in err loc Unterminated_comment
      | _ ->
          match Stream.peek strm__ with
            Some ('\n' as z) ->
              Stream.junk strm__;
          ...
          | Some z ->
              Stream.junk strm__;
              let s = strm__ in real_push_char z; comment loc bp s
...
let rec progress_further loc last nj tt cs =
  try
    progress_from_byte loc last nj tt cs
      (List.nth (Stream.npeek (nj + 1) cs) nj)
  with Failure _ -> last
and update_longest_valid_token loc last nj tt cs =
  match tt.node with
    Some _ as last' -> stream_njunk nj cs; progress_further loc last' 0 tt cs
  | None -> progress_further loc last nj tt cs
and progress_utf8 loc last nj n c tt cs =
  try
    let tt = CharMap.find c tt.branch in
    if Int.equal n 1 then update_longest_valid_token loc last (nj + n) tt cs
    else
...
let process_chars loc bp c cs =
  ...
  | None ->
      ...
      njunk (ep' - ep) cs;
      let loc = set_loc_pos loc bp ep' in err loc Undefined_token

(* Parse what follows a dot *)

let parse_after_dot loc c bp (strm__ : _ Stream.t) =
  match Stream.peek strm__ with
    Some ('a'..'z' | 'A'..'Z' | '_' as d) ->
      Stream.junk strm__;
      let len =
        try ident_tail loc (store 0 d) strm__ with
...
let rec next_token loc (strm__ : _ Stream.t) =
  ...
  | Some (' ' | '\t' | '\r' as c) ->
      Stream.junk strm__;
      let s = strm__ in comm_loc bp; push_char c; next_token loc s
  | Some ('.' as c) ->
      Stream.junk strm__;
      let t =
        try parse_after_dot loc c bp strm__ with
          Stream.Failure -> raise (Stream.Error "")
      in
      let s = strm__ in
      let ep = Stream.count strm__ in
  ...
  | Some ('a'..'z' | 'A'..'Z' | '_' as c) ->
      Stream.junk strm__;
      let len =
        try ident_tail loc (store 0 c) strm__ with
          Stream.Failure -> raise (Stream.Error "")
      in
      let s = strm__ in
      let ep = Stream.count strm__ in
  ...
  | Some ('0'..'9' as c) ->
      ...
      in
      let ep = Stream.count strm__ in
      comment_stop bp; INT (get_buff len), set_loc_pos loc bp ep
  | Some '\"' ->
      Stream.junk strm__;
      let (loc, len) =
        try string loc ~comm_level:None bp 0 strm__ with
          Stream.Failure -> raise (Stream.Error "")
      in
      let ep = Stream.count strm__ in
      comment_stop bp; STRING (get_buff len), set_loc_pos loc bp ep
  | Some ('(' as c) ->
      Stream.junk strm__;
      begin try
        match Stream.peek strm__ with
        ...
        | _ -> let t = process_chars loc bp c strm__ in comment_stop bp; t
      with Stream.Failure -> raise (Stream.Error "")
      end
  | Some ('{' | '}' as c) ->
      Stream.junk strm__;
  ...
  | _ ->
      ...
      | AsciiChar | Utf8Token _ ->
          let t = process_chars loc bp (Stream.next s) s in comment_stop bp; t
      | EmptyStream -> comment_stop bp; EOI, set_loc_pos loc bp (bp + 1)
...
let loct_func loct i = try Hashtbl.find loct i with Not_found -> locerr ()
  ...
  let ts =
    Stream.from
      (fun i ->
         ...
         cur_loc := after loc; loct_add loct i loc; Some tok)
  in
  ts, loct_func loct
...
let lexer =
  {Token.tok_func = func;
   Token.tok_using =
     (fun pat ->
        match Tok.of_pattern pat with
          KEYWORD s -> add_keyword s
        | _ -> ());
   Token.tok_removing = (fun _ -> ()); Token.tok_match = Tok.match_pattern;
   Token.tok_comm = None; Token.tok_text = token_text}

(** Terminal symbols interpretation *)

let is_ident_not_keyword s = is_ident s && not (is_keyword s)
