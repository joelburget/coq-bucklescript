...
let subst_command_placeholder s t =
  let buff = Buffer.create (Js.String.length s + Js.String.length t) in
  let i = ref 0 in
  while (!i < Js.String.length s) do
    if s.[!i] = '%' && !i+1 < Js.String.length s && s.[!i+1] = 's'
...
let matrix_transpose mat = failwith "undefined: matrix_transpose"
  (*
