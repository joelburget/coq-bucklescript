...
let print_path_entry p = failwith "undefined: print_path_entry"
  (*
...
let print_loadpath dir = failwith "undefined: print_loadpath"
(*
...
let print_modules () = failwith "undefined: print_modules"
(*
...
let locate_file f = failwith "undefined: locate_file"
...
let print_located_library r = failwith "undefined: print_located_library"
(*
...
let smart_global r = failwith "undefined: smart_global"
  (*
...
let vernac_definition ~atts discharge kind ({loc;v=id}, pl) def = failwith "undefined: vernac_definition"
(*
...
let vernac_start_proof ~atts kind l = failwith "undefined: vernac_start_proof"
(*
...
let vernac_assumption ~atts discharge kind l nl = failwith "undefined: vernac_assumption"
(*
  let local = enforce_locality_exp atts.locality discharge in
...
let vernac_record cum k poly finite struc binders sort nameopt cfs = failwith "vernac_record"
(*
...
(** When [poly] is true the type is declared polymorphic. When [lo] is true,
    ...
    neither. *)
let vernac_inductive ~atts cum lo finite indl = failwith "vernac_inductive"
(*
...
let vernac_fixpoint ~atts discharge l = failwith "vernac_fixpoint"
(*
  let local = enforce_locality_exp atts.locality discharge in
  if Dumpglob.dump () then
...
let vernac_cofixpoint ~atts discharge l = failwith "vernac_cofixpoint"
(*
  let local = enforce_locality_exp atts.locality discharge in
  if Dumpglob.dump () then
...
let vernac_scheme l = failwith "vernac_scheme"
(*
  if Dumpglob.dump () then
    List.iter (fun (lid, s) ->
...
let vernac_combined_scheme lid l = failwith "vernac_combined_scheme"
(*
  if Dumpglob.dump () then
...
let vernac_define_module export {loc;v=id} (binders_ast : module_binder list) mty_ast_o mexpr_ast_l = failwith "vernac_define_module"
(*
  (* We check the state of the system (in section, in module type)
     and what module information is supplied *)
  if Lib.sections_are_opened () then
...
let vernac_end_module export {loc;v=id} = failwith "vernac_end_module"
(*
...
let vernac_declare_module_type {loc;v=id} binders_ast mty_sign mty_ast_l = failwith "vernac_declare_module_type"
(*
  if Lib.sections_are_opened () then
    user_err Pp.(str "Modules and Module Types are not allowed inside sections.");
...
let vernac_end_modtype {loc;v=id} = failwith "vernac_end_modtype"
(*
...
let vernac_begin_section ({v=id} as lid) = failwith "vernac_begin_section"
(*
  Proof_global.check_no_pending_proof ();
...
let vernac_end_section {CAst.loc} = failwith "vernac_end_section"
(*
...
let vernac_instance ~atts abst sup inst props pri = failwith "vernac_instance"
(*
...
let vernac_syntactic_definition ~atts lid x y = failwith "vernac_syntactic_definition"
(*
